...
model_categories = ['prost', 'no_dist', 'kl_decay', 'kl', 'no_kl', 'random']
#model_categories = ['prost', 'no_dist', 'no_kl', 'kl', 'random']
#model_categories = ['prost', 'no_dist', 'no_kl', 'kl_decay', 'random']
output_filename = 'result_csvs/lr_camera_ready_results.csv'
std_dev = False
take_max = False
all_entries = False
...
def get_score(domain, models, instances):
    ...
    for model in models:
        if model != 'random' and model != 'prost':
            ptr = open(f'/scratch/cse/dual/cs5180404/expt/uai2023_camera_ready/lr/{model}/{domain}10x10-15x15-symnet3prost/testing_200.csv')
        elif model == 'random':
