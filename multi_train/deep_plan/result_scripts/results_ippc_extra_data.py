...
def get_score(domain, models, instances):
    ...
    for model in models:
        if model != 'random' and model != 'prost':
            ptr = open(f'/scratch/cse/dual/cs5180404/expt/icaps2023_after/ippc_rerun/{model}/{domain}10x10-15x15-symnet3prost/testing_200.csv')
        elif model == 'random':
...
with open('ippc_extra_data_results.csv', 'w') as output_file:
