...
model_categories = ['prost', 'no_dist', 'kl_no_decay', 'no_kl', 'kl_decay']
output_csv = 'result_csvs/lr_kl_decay_results.csv'
std_dev = False
def get_score(domain, models, instances):
    ...
    for model in models:
        if model != 'random' and model != 'prost':
            ptr = open(f'/scratch/cse/dual/cs5180404/expt/uai2023_after/lr_full_kl_decay/{model}/{domain}10x10-15x15-symnet3prost/testing_200.csv')
        elif model == 'random':
    ...
    for model in models:
        for cat in model_categories:
            if model.startswith(cat+"_run" if model not in ['prost', 'random'] else cat):
...
models = ['no_dist_run1', 'no_dist_run2', 'no_dist_run3', 'kl_no_decay_run1', 'kl_no_decay_run2', 'kl_no_decay_run3', 'no_kl_run1', 'no_kl_run2', 'no_kl_run3', 'kl_decay_run1', 'kl_decay_run2', 'kl_decay_run3', 'prost', 'random']
